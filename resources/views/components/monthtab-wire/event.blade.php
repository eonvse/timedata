...
<a href="{{ route('info.time-event',['id'=>$id, 'edit'=>0])}}" class="text-indigo-900  flex flex-row items-center h-5 px-1 text-xs hover:bg-gray-500 hover:text-white {{ $color }}" title="{{ $start }} {{ !empty($end) ? '- '.$end : $end }}: {{ $title }}">
